---
layout: default
title: Mert Melih Albay
---

<p>
  <strong>Ana Sayfa</strong> | 
  <a href="projects.html">Projelerim</a> | 
  <a href="en/index.html" class="lang-switcher">🌐 View in English</a>
</p>

<hr>

<p>


<h2 style="color: #2e81b7;">HAKKIMDA</h2>
<p>
  Donanım mimarisi, dijitalizasyon, gömülü sistemler, elektronik devre tasarımı ve 
  gerçek zamanlı veri füzyonu gibi alanlarda iki yıldan fazla pratik deneyime sahip 
  <strong>Elektrik & Elektronik Mühendisliği</strong> son sınıf öğrencisi. Yapay zeka tabanlı analiz 
  yöntemleriyle birleşik otonom sistemler geliştirdim. Bir projeyi tasarım aşamasından 
  prototip üretimine kadar bağımsız yürütebilirim.
</p>

<h2 style="color: #2e81b7;">EĞİTİM</h2>
<p>
  <strong>Muğla Sıtkı Koçman Üniversitesi</strong> | 2022 - Devam Ediyor <br>
  Elektrik Elektronik Mühendisliği (İngilizce) - 4. Sınıf
</p>

<h2 style="color: #2e81b7;">DENEYİM</h2>

<div style="margin-bottom: 20px;">
  <h3 style="margin-bottom: 5px;">Kalyon Enerji Yatırımları A.Ş.</h3>
  <em>Stajyer | 07/07/2025 - 04/08/2025</em>
  <ul>
    <li><strong>AutoCAD</strong> kullanarak güneş enerjisi santrallerinde saha yerleşim planlaması gerçekleştirdim.</li>
    <li><strong>PVsyst</strong> ve <strong>MATLAB</strong> ile gölgelenme ve üretim verimliliği simülasyonları modelledim.</li>
    <li>Rüzgar türbinlerinden elde edilen gerçek zamanlı <strong>SCADA</strong> verilerini <strong>Python</strong> ile analiz ederek güç tahmini ve performans değerlendirmesi için matematiksel modeller geliştirdim.</li>
  </ul>
</div>

<div style="margin-bottom: 20px;">
  <h3 style="margin-bottom: 5px;">Chemnitz Teknoloji Üniversitesi | Almanya</h3>
  <em>Akademik Araştırma Programı (Planlanan) | Ağustos 2026</em>
  <ul>
    <li>Prof. Dr. Florian Röhrbein'in resmi davetiyle <strong>"Nörorobotik ve Akıllı Sistemler"</strong> alanında bir aylık akademik araştırma programına kabul edildim.</li>
    <li>Laboratuvar bünyesindeki araştırma faaliyetlerine katılacak ve devam eden akıllı sistem projelerinin dinamiklerini inceleyeceğim.</li>
  </ul>
</div>

<h2 style="color: #2e81b7;">YABANCI DİL</h2>
<ul>
  <li><strong>İNGİLİZCE:</strong> C1 (İleri Seviye)</li>
</ul>

<h2 style="color: #2e81b7;">BECERİLER</h2>
<p>
  <strong>Yazılım ve Araçlar:</strong> MATLAB, Altium Designer, Python, VHDL, AutoCAD, PVsyst <br>
  <strong>Teknik Odak:</strong> Gömülü Sistemler, RF, Sinyal İşleme, Elektromekanik, Görüntü İşleme, Sensör Füzyonu
</p>

<h2 style="color: #2e81b7;">SERTİFİKA VE LİSANS</h2>
<ul>
  <li><strong>General Electric Aerospace:</strong> Explore Engineering Job Simulation | 2024</li>
  <li><strong>MITx (edX):</strong> Circuits And Electronics: Circuit Analysis I | 2023</li>
  <li><strong>Dijital Donanım Tasarımcılığı:</strong> FPGA / ASIC Tasarım Kursu | 2026</li>
  <li><strong>Sivil Havacılık Genel Müdürlüğü:</strong> Sportif/Amatör İHA Lisansı</li>
</ul>

<hr>

<p>
  <a href="assets/MertMelihAlbay_CV.pdf" class="cv-button" download>Özgeçmişimi İndir (PDF)</a>
