---
layout: default
title: Mert Melih Albay
---

<p>
  <strong>Ana Sayfa</strong> | 
  <a href="projects.html">Projelerim</a> | 
  <a href="en/index.html" class="lang-switcher">🌐 EN </a>
</p>

<hr>

<p>


<h2 style="color: #2e81b7;">HAKKIMDA</h2>
<p>
  Donanım mimarisi, dijitalizasyon, gömülü sistemler, elektronik devre tasarımı ve 
  gerçek zamanlı veri füzyonu gibi alanlarda iki yıldan fazla pratik deneyime sahip 
  Elektrik & Elektronik Mühendisliği son sınıf öğrencisi. Yapay zeka tabanlı analiz 
  yöntemleriyle birleşik otonom sistemler geliştirdim. Bir projeyi tasarım aşamasından 
  prototip üretimine kadar bağımsız yürütebilirim.
</p>

<h2 style="color: #2e81b7;">EĞİTİM</h2>
<p>
  Muğla Sıtkı Koçman Üniversitesi | 2022 - Devam Ediyor <br>
  Elektrik Elektronik Mühendisliği (İngilizce) - 4. Sınıf
</p>

<h2 style="color: #2e81b7;">DENEYİM</h2>

<div style="margin-bottom: 20px;">
  <h3 style="margin-bottom: 5px;">Kalyon Enerji Yatırımları A.Ş.</h3>
  <em>Stajyer | 07/07/2025 - 04/08/2025</em>
  <ul>
    <li>AutoCAD kullanarak güneş enerjisi santrallerinde saha yerleşim planlaması gerçekleştirdim.</li>
    <li>PVsyst ve MATLAB ile gölgelenme ve üretim verimliliği simülasyonları modelledim.</li>
    <li>Rüzgar türbinlerinden elde edilen gerçek zamanlı SCADA verilerini Python ile analiz ederek güç tahmini ve performans değerlendirmesi için matematiksel modeller geliştirdim.</li>
  </ul>
</div>

<div style="margin-bottom: 20px;">
  <h3 style="margin-bottom: 5px;">Chemnitz Teknoloji Üniversitesi | Almanya</h3>
  <em>Akademik Araştırma Programı (Planlanan) | Ağustos 2026</em>
  <ul>
    <li>Prof. Dr. Florian Röhrbein'in resmi davetiyle "Nörorobotik ve Akıllı Sistemler" alanında bir aylık akademik araştırma programına kabul edildim.</li>
    <li>Laboratuvar bünyesindeki araştırma faaliyetlerine katılacak ve devam eden akıllı sistem projelerinin dinamiklerini inceleyeceğim.</li>
  </ul>
</div>

<h2 style="color: #2e81b7;">YABANCI DİL</h2>
<ul>
  <li>İNGİLİZCE: C1 </li>
</ul>

<h2 style="color: #2e81b7;">BECERİLER</h2>
<p>
MATLAB | Altium Designer | Python | VHDL | AutoCAD | PVsyst | Gömülü Sistemler | RF
Sinyal İşleme | Elektromekanik | Görüntü İşleme | Sensör Füzyonu
</p>

<h2 style="color: #2e81b7;">SERTİFİKA VE LİSANS</h2>
<ul>
  <li>General Electric Aerospace: Explore Engineering Job Simulation | 2024</li>
  <li>MITx (edX): Circuits And Electronics: Circuit Analysis I | 2023</li>
  <li>Dijital Donanım Tasarımcılığı: FPGA / ASIC Tasarım Kursu | 2026</li>
  <li>Sivil Havacılık Genel Müdürlüğü: Sportif/Amatör İHA Lisansı</li>
</ul>

<hr>

<p>
  <a href="assets/MertMelihAlbay_CV.pdf" class="cv-button" download>Özgeçmişimi İndir (PDF)</a>
